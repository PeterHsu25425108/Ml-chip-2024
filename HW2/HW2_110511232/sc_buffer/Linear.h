...
SC_MODULE(Flatten)
{
    ...
    SC_CTOR(Flatten)
    {
        in_data = new sc_port<sc_signal_in_if<double>> **[256];
        for (int i = 0; i < 256; i++)
        {
            in_data[i] = new sc_port<sc_signal_in_if<double>> *[6];
            for (int j = 0; j < 6; j++)
            {
                in_data[i][j] = new sc_port<sc_signal_in_if<double>>[6];
            }
        }
        out_data = new sc_port<sc_signal_out_if<double>>[256 * 6 * 6];
...
SC_MODULE(Linear)
{
    ...
    void run()
    {
        // add the product of input and weight to the variable sum
    ...
    Linear(sc_module_name name, int in_size, int out_size, string file_name) : in_size(in_size), out_size(out_size), file_name(file_name)
    {
        ...
        in_data = new sc_port<sc_signal_in_if<double>>[in_size];
        out_data = new sc_port<sc_signal_out_if<double>>[out_size];

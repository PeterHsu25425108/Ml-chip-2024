...
// Pattern module reads input data from a file and writes it to the output ports
SC_MODULE(Pattern)
{
    string infile_name;
    ...
    Pattern(sc_module_name name, string infile_name) : infile_name(infile_name)
    {
        ...
        // Parse the input data and store it in in_data
        double val;
        for (int i = 0; i < 3; i++)
        {
            for (int j = 0; j < 224; j++)
        ...
        out_data = new sc_port<sc_signal_out_if<double>> **[3];
        for (int i = 0; i < 3; i++)
        {
            out_data[i] = new sc_port<sc_signal_out_if<double>> *[224];
            for (int j = 0; j < 224; j++)
            {
                out_data[i][j] = new sc_port<sc_signal_out_if<double>>[224];
            }
        }
        SC_METHOD(run);
        sensitive << iclk.pos();
